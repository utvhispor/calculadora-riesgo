let notaClase = '<strong>Frecuencia respiratoria</strong><br /><br />'
notaClase += 'Usando la mitad de la actividad sedentaria y ligera, 16-21 años por defecto. La frequencia respiratoria solo importa para personas susceptibles. Para estudiar la transmisión de los estudiantes al maestro, es necesario aumentar este valor en consecuencia.<br />'
notaClase += 'Para estudiar la transmisión a niños pequeños, la frequencia respiratoria también debe ajustarse de acuerdo con la tabla Readme.<br /><br />'
notaClase += '<strong>Tasa de emisión cuantica</strong><br /><br />'
notaClase += 'El valor actual es para una profesora hablando mucho.<br />'
notaClase += 'Para los adolescentes, se usan los valores del archivo Readme, según la actividad.<br />'
notaClase += 'Se cree que los niños pequeños son menos contagiosos y también respiran menos aire, por lo que quizás reduzca la Tasa de emisión cuantica en x5.'

let notaCoro = 'La probabilidad de muerte se establece más alta debido a la mayor edad de los miembros del coro (el 75% de los enfermos tenían >= 65 años). Sugerir cambiar al 1% para aplicaciones generales.<br /><br />'
notaCoro += 'Probabilidad de ser infeccioso calculada a partir de la prevalencia de la enfermedad en el condado en ese momento (7 casos * 2 para dar cuenta de los casos no detectados / 129000 habitantes del condado), ver Miller et al. 2020.<br /><br />'

let notaEstadio = 'Este es un caso para un estadio de fútbol, como se explica en este tweet: <a href="https://twitter.com/jljcolorado/status/1289254898131820550">https://twitter.com/jljcolorado/status/1289254898131820550</a><br /><br />'
notaEstadio += 'E basado en este articulo: <a href="https://www.sciencedirect.com/science/article/pii/S1352231013004494">https://www.sciencedirect.com/science/article/pii/S1352231013004494</a><br /><br />'
notaEstadio += 'Tener en cuenta que para el riesgo de infección, la calculadora solo estima el riesgo de infección a través del aire más allá de las proximidades. Dado que la distancia social no se mantendría en tal evento, esto será una gran subestimación.'

let notaSuper = 'Basado en un supermercado específico en Boulder, Colorado.<br /><br />'
notaSuper +='Dimensiones horizontales estimadas de Google Maps (usando escala), altura usando imágenes de Google Street View (usando personas presentes para escala).<br /><br />'
notaSuper +='Tasa de ventilación estimada a partir del estándar ASHRAE en la página Readme.<br /><br />'
notaSuper +='Promedio diario típico de ocupación, basado en mis visitas al espacio antes de la pandemia (puede ser menor ahora).<br /><br />'
notaSuper +='Otros parámetros estimados en el Readme para esta situación.<br /><br />'
notaSuper +='Esto es para un trabajador de un supermercado. Para un cliente, se cambia el tiempo dedicado a la historia a, por ejemplo, 1 hora, 4 veces a la semana para simular 1 mes.'

let infoAmbientales = '<strong>Largo</strong> - Puede ingresar como "pies" o como "m" (una vez ingresado como "m", cambiar en "pies" no funciona).<br /><br />'
infoAmbientales += '<strong>Ancho</strong> - Puede ingresar como "pies" o como "m" (una vez ingresado como "m", cambiar en "pies" no funciona).<br /><br />'
infoAmbientales += '<strong>Alto</strong> - Puede ingresar como "pies" o como "m" (una vez ingresado como "m", cambiar en "pies" no funciona).<br /><br />'
infoAmbientales += '<strong>Duracion evento</strong> - Valor para su situación de interés.<br /><br />'
infoAmbientales += '<strong>Nº rep. evento</strong> - Por ejemplo múltiples reuniones de clase, múltiples desplazamientos en transporte público, etc.<br /><br />'
infoAmbientales += '<strong>Cambios de aire</strong> - Valor en h-1: Readme: Igual que "cambios de aire por hora". Valor en L/s/per para comparar con las pautas (por ejemplo, ASHRAE 62.1).<br /><br />'
infoAmbientales += '<strong>Presión</strong> - Utilizado solo para el cálculo de CO2.<br /><br />'
infoAmbientales += '<strong>Temperatura</strong> - Use el <a href="https://www.google.com/search?q=fahrenheit+to+c+converter" target="blank">convertidor web</a> si es necesario para convertir Fahrenheit -> Celsius. Se usa para el cálculo de CO2, eventualmente para la tasa de supervivencia del virus.<br /><br />'
infoAmbientales += '<strong>Humedad</strong> - Aún no se usa, pero eventualmente se puede usar para la tasa de supervivencia del virus.<br /><br />'
infoAmbientales += '<strong>CO2 al aire libre</strong> - Ver Readme<br /><br />'
infoAmbientales += '<strong>Tasa descomposicón</strong> - Ver Readme: puede estimar para una determinada T, RH, UV a partir del estimador DHS.<br /><br />'
infoAmbientales += '<strong>Deposicón superficies</strong> - Buonnano y col. (2020), Miller et al. (2020). Puede variar de 0,24 a 1,5 h-1, según el rango de tamaño de partícula.<br /><br />'
infoAmbientales += '<strong>Medidas control adicionales</strong> - Por ejemplo filtrado de recirculacion de aire, filtro de aire HEPA, desinfección UV, etc. Consulte las preguntas frecuentes, el archivo Readme de calculo para filtro HEPA portátil.<br /><br />'
infoAmbientales += '<strong>Tasa de pérdida total de primer orden</strong> - Suma de todas las tarifas de primer orden.<br /><br />'
infoAmbientales += '<strong>Tasa de ventilación por persona</strong> - Este es el valor de la ventilación que realmente importa para la transmisión de enfermedades. Incluye medidas de control adicionales.<br /><br />'

let infoPersonas = '<strong>Nº Personas</strong> - Valor para su situación de interés.<br /><br />'
infoPersonas += '<strong>Población inmune</strong> - De los informes de seroprevalencia, dependerá de cada ubicación y hora, consulte el archivo Readme.<br /><br />'
infoPersonas += '<strong>Personas susceptibles</strong> - Valor para su situación de interés.<br /><br />'
infoPersonas += '<strong>Frec. respiratoria</strong> - Ver Readme. Varía mucho según el nivel de actividad.<br /><br />'
infoPersonas += '<strong>Exhalación cuántica</strong> - Ver Reame. Depende mucho de la actividad, también como persona. Este es el parámetro más incierto, pruebar con diferentes valores.<br /><br />'
infoPersonas += '<strong>Ef. mascarilla exhalación</strong> - 0 si la persona infectada no lleva máscara. Ver Readme.<br /><br />'
infoPersonas += '<strong>Personas con mascarillas</strong> - Valor para su situación. Se aplica a todo el mundo tanto para la emisión como para la inhalación. Modificar fórmulas manualmente si es necesario.<br /><br />'
infoPersonas += '<strong>Ef. mascarilla inhalación</strong> - Ver Readme.<br /><br />'
infoPersonas += '<strong>Pers. infecciosas</strong> - Mantenga esto en uno a menos que realmente desee para estudiar casos diferentes; vea los resultados condicionales y absolutos.<br /><br />'
infoPersonas += '<strong>Tasa CO2 por persona</strong> - Ver Readme. Esto no afecta el cálculo de la infección, solo el uso de CO2 como indicador, se podría ignorar.<br /><br />'
infoPersonas += '<strong>Tasa CO2 para todas las personas</strong> - El anterior multiplicado por el número de personas y aplicando la ley de los gases ideales para convertir a P&T ambiental.<br /><br />'

let infoCovid = '<strong>Probabilidad de ser infeccioso</strong> - Parámetro muy importante, específico para cada región y período de tiempo. Para resultados ABSOLUTOS (probablemente dada la prevalencia de la enfermedad en la población). Ver Readme.<br /><br />'
infoCovid += '<strong>Tasa de hospitalización</strong> - De informes de noticias. Varía mucho con la edad y los factores de riesgo.<br /><br />'
infoCovid += '<strong>Índice de mortalidad</strong> - De informes de noticias. Varía mucho con la edad y los factores de riesgo (1% típico - más alto para personas mayores / en riesgo).<br /><br />'

let infoResultadoCondicional = '<strong>Tasa de emisión neta</strong> - Incluye el número de personas infectadas presentes.<br /><br />'
infoResultadoCondicional += '<strong>Concentración media cuantica</strong> - Solución analítica del modelo de caja. La ecuación (4) de Miller et al.(2020)<br /><br />'
infoResultadoCondicional += '<strong>Prob. infección</strong> - Aplicando el modelo de infección de Wells-Riley a la cantidad de dosis infecciosas inhaladas. La ecuación (1) de Miller et al.(2020)<br /><br />'
infoResultadoCondicional += '<strong>Relación con el riesgo de muerte por viaje en automóvil</strong> - Consulte las FAQ para obtener una estimación aproximada de las muertes por viaje en automóvil en un día determinado.<br /><br />'
infoResultadoCondicional += '<strong>Casos COVID surgidos</strong> - Número de personas. Multiplica la probabilidad de una persona por el número de personas susceptibles presentes.<br /><br />'
infoResultadoCondicional += '<strong>Hospitalizaciones surgidas</strong> - Número de personas.<br /><br />'
infoResultadoCondicional += '<strong>Muertes surgidas</strong> - Número de personas.<br /><br />'
infoResultadoCondicional += '<strong>Proporción media de mezcla de CO2</strong> - Solución analítica del modelo de caja. La ecuación (4) de Miller et al.(2020). Consulte las FAQ para conocer las diferencias con la calculadora cuantica.<br /><br />'

let infoResultadoAbsolutoEvento = '<strong>Casos COVID surgidos</strong> - Debe interpretarse estadísticamente. Este sería el promedio por ejemplo. 100 repeticiones del evento en un lugar determinado.<br /><br />'
infoResultadoAbsolutoEvento += '<strong>Relación con el riesgo de muerte por viaje en automóvil</strong> - Consulte las FAQ para obtener una estimación aproximada de las muertes por viaje en automóvil en un día determinado.<br /><br />'
infoResultadoAbsolutoEvento += '<strong>Casos COVID surgidos</strong> - Número de personas.<br /><br />'
infoResultadoAbsolutoEvento += '<strong>Hospitalizaciones surgidas</strong> - Número de personas.<br /><br />'
infoResultadoAbsolutoEvento += '<strong>Muertes surgidas</strong> - Número de personas.<br /><br />'
infoResultadoAbsolutoEvento += '<strong>Proporción media de mezcla de CO2</strong> - Solución analítica del modelo de caja. La ecuación (4) de Miller et al.(2020). Consulte las FAQ para conocer las diferencias con la calculadora cuantica.<br /><br />'

let infoResultadoAbsolutoEventoMultiples = '<strong>Relación con el riesgo de muerte por viaje en automóvil</strong> - Consulte las FAQ para obtener una estimación aproximada de las muertes por viaje en automóvil en un día determinado.<br /><br />'
infoResultadoAbsolutoEventoMultiples += '<strong>Casos COVID surgidos</strong> - Número de personas.<br /><br />'
infoResultadoAbsolutoEventoMultiples += '<strong>Hospitalizaciones surgidas</strong> - Número de personas.<br /><br />'
infoResultadoAbsolutoEventoMultiples += '<strong>Muertes surgidas</strong> - Número de personas.<br /><br />'
